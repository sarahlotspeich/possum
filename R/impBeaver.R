#' Multiple imputation for log-binomial regression problems with covariate measurement error
...
#' @param analysis_formula analysis model formula (or coercible to formula) passed through to \code{glm()}, a formula expression as for other regression models. The response should be the log-binomial model outcome as \code{cbind(cases, controls)}.
...
impBeaver = function(imputation_formula, analysis_formula, data, adjMatrix = NULL, B = 1, seed = NULL) {
  ...
  if (is.null(adjMatrix)) {
    cc_fit =  glm(formula = analysis_formula,
                  ...
                  data = data)
    # cc_fit = logbin(formula = analysis_formula,
    #                 data = data)
    ...
    if (is.null(adjMatrix)) {
      res = data.frame(Coefficient = names(coefficients(fit)),
                       ...
                       Standard.Error = sapply(X = 1:dim_beta,
                                               FUN = function(c) sqrt(mean(imp_vars[, c]) + (B + 1) / (B - 1) * mean((imp_params[, c] - mean(imp_params[, c])) ^ 2))))
    } else {
