...
#' This function returns the sieve maximum likelihood estimates (SMLEs) for the zero-inflated negative binomial regression model with covariate measurement error from Lotspeich et al. (2025+)
#'
...
zeroinfl_smlePossum_nb = function(analysis_formula, error_formula, data, no_se = TRUE, pert_scale = 1,
  ...
  C = setdiff(x = unlist(strsplit(x = gsub(pattern = " ",
                                  ...
                                  split = "+",
                                  fixed = TRUE)),
              y = X_val) ## additional error-free covariates in count model
  ...
  if(no_se){
    ...
    if(output == "coeff") {
      ...
      od_loglik_conv = zeroinfl_smlePossum_negbin_od_ll(
        N = N,
        n = n,
        prev_beta_fit = M_step_res$new_beta_fit,
        Y = Y,
