#' (Archived) Maximum likelihood estimation for Poisson regression problems with covariate misclassification
...
archived_mlePossum = function(error_formula, analysis_formula, offset = NULL, data, noFN = FALSE) {
  ...
  len_optim <- length(optim_res$par)
  error_start <- num_analysis_covar + 1
  ...
  return(list(coefficients = res,
              misclass_coefficients = error_coefs,
