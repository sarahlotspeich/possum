...
smlePossum_nb = function(analysis_formula, error_formula, data, no_se = TRUE, pert_scale = 1,
  ...
  C = setdiff(x = unlist(strsplit(x = gsub(pattern = " ",
                                  ...
                                  fixed = TRUE)),
              y = X_val)
  if (length(C) == 1 & any(grepl(pattern = "offset", x = C))) {
    C = NULL
  } else {
    C = C[-grepl(pattern = "offset", x = C)] ## remove offset from covar vector   
  }
  offset = sub(pattern = "\\).*",
               replacement = "",
  ...
  comp_dat_unval = suppressWarnings(
    data.matrix(
      cbind(data[-c(1:n), c(Y, offset, C, Bspline)],
            x_obs_stacked,
            k = rep(seq(1, m), each = (N - n)))
    )
  )
  comp_dat_unval = cbind(comp_dat_unval, int = 1) ### add intercept column
  comp_dat_unval = comp_dat_unval[, c("int", Y, offset, X_val, C, Bspline, "k")]
